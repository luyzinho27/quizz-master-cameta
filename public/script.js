                ...
                .then(querySnapshot => {
                    ...
                    querySnapshot.forEach(doc => {
                        ...
                        quizResults.push({
                            ...
                            percentage: userQuiz.percentage || 0,
                            timeTaken: userQuiz.timeTaken || 0,
                            completedAt: userQuiz.completedAt || userQuiz.updatedAt
                        });
                    });
...
function loadFullRanking() {
    ...
    db.collection('userQuizzes')
        ...
        .then(userQuizzesSnapshot => {
            ...
            if (userIds.length === 0) {
                fullRankingElement.innerHTML = '<p>Nenhum dado disponível para exibir.</p>';
                return;
            }
...
function showDashboard() {
    ...
    if (currentUser.userType === 'admin') {
        ...
        adminDashboard.classList.add('hidden');
        document.getElementById('professor-name').textContent = currentUser.name;
        // Carregar dados específicos do professor, se necessário
    } else {
        adminDashboard.classList.add('hidden');
        studentDashboard.classList.remove('hidden');
        document.getElementById('student-name').textContent = currentUser.name;
